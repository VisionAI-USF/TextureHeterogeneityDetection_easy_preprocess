...
cid = unique(cluster_idx{1});
if length(cid) == 1
    habitats{1} = zeros(1,size(mask,1),size(mask,1));
    habitats{1}(1,:,:) = mask>1;
    return
end
...
for scale = 1:length(cid)
    ...
    coord_idx = find(cluster_idx{1}==idx);
    for j = 1:length(coord_idx)
        
        habitats{1}(scale,patches(coord_idx(j),1),patches(coord_idx(j),2))=idx;
